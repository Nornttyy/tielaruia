...
const _P_COAL_ORE := {
	"s": Color8(108, 100, 92),   # 石灰底 (比矿色亮, 让黑煤块跳出来)
	"S": Color8(84, 78, 71),
	"l": Color8(132, 123, 113),
	"k": Color8(58, 53, 48),
	"L": Color8(150, 140, 129),
	"m": Color8(96, 89, 81),
	"b": Color8(72, 66, 60),
	"o": Color8(114, 105, 96),
	"c": Color8(40, 36, 34),     # 煤黑
	"C": Color8(22, 20, 19),     # 极黑
	"h": Color8(58, 52, 48),     # 煤中
	"_o": Color8(40, 36, 32),
	"_e": Color8(78, 70, 63),
	"_h": Color8(150, 140, 129),
	"_H": Color8(170, 160, 148),
}

# 铁矿: STONE 底色 + r/R/H 系列铁锈
const _P_IRON_ORE := {
	"s": Color8(120, 110, 100), "S": Color8(92, 84, 76),
	"l": Color8(146, 136, 124), "k": Color8(64, 58, 52),
	"L": Color8(162, 152, 140), "m": Color8(106, 97, 88),
	"b": Color8(80, 72, 65), "o": Color8(126, 115, 103),
	"h": Color8(110, 100, 90),
	"r": Color8(175, 75, 42),     # 铁锈红橙 (比铜更红更暗, 好区分)
	"R": Color8(128, 48, 22),
	"H": Color8(212, 108, 62),
	"_o": Color8(40, 36, 32), "_e": Color8(78, 70, 63),
	"_h": Color8(158, 148, 136), "_H": Color8(178, 168, 155),
}
...
const _P_COPPER_ORE := {
	...
	"b": Color8(80, 72, 65), "o": Color8(126, 115, 103),
	"h": Color8(110, 100, 90),
	"u": Color8(235, 135, 52),    # 铜亮橙金 (比铁更亮更黄, 好区分)
	"U": Color8(175, 78, 24),
	"p": Color8(255, 185, 105),
	"_o": Color8(40, 36, 32), "_e": Color8(78, 70, 63),
	"_h": Color8(158, 148, 136), "_H": Color8(178, 168, 155),
}
...
const _P_TIN_ORE := {
	...
	"b": Color8(80, 72, 65), "o": Color8(126, 115, 103),
	"h": Color8(110, 100, 90),
	"n": Color8(165, 200, 222),   # 锡偏蓝白 (跟银的纯白区分)
	"N": Color8(115, 148, 172),
	"x": Color8(225, 245, 253),
	"_o": Color8(40, 36, 32), "_e": Color8(78, 70, 63),
	"_h": Color8(158, 148, 136), "_H": Color8(178, 168, 155),
}
...
const _P_CUTTING_BOARD := {
	".": Color(0, 0, 0, 0),       # 透明
	"h": Color8(170, 120, 70),    # 木板浅
	"H": Color8(120, 80, 45),     # 木板深 (边)
	"A": Color8(220, 90, 80),     # 鱼片红
}
...
# 树干顶帽: 窄树干 (同 _LOG 宽度) + 顶上一条暗 "圆环 cap" 让树干顶部不生硬
const _LOG_TOP := [
	"....BBBBBBBB....",
	"....BlrblrbB....",
	"....BblrblbB....",
...
# 树根 左侧: 窄树干 (col 4-11) 在上, 底部一条根往左下扎出
const _LOG_ROOT_L := [
	"....BblrblbB....",
	"....BblrblbB....",
	"....BblrblbB....",
...
# 左侧枝: 窄树干 (col 4-11) + 左边一小撮叶簇贴着树干 (树干变细后左边空出来了)
const _BRANCH_L := [
	"....BblrblbB....",
	"....BblrblbB....",
	"....BblrblbB....",
...
# 窄树干 (用户要求"细一点"): 树干只占中间 8px (col 4-11), 左右各 4px 透明 = 像真树干不是整块。
# 竖纹树皮 (B 暗边/凹沟 + l 高光 + r 中调) + 2 个木结 (RR)。LOG_TOP/枝/根 统一这个宽度衔接。
const _LOG := [
	"....BblrblbB....",
	"....BblrblbB....",
	"....BblrblbB....",
...
const _P_SILVER_ORE := {
	...
	"L": Color8(162, 152, 140), "m": Color8(106, 97, 88),
	"b": Color8(80, 72, 65), "o": Color8(126, 115, 103),
	"v": Color8(244, 247, 253),   # 银纯亮白 (跟锡的蓝白区分)
	"V": Color8(198, 208, 224),
	"_o": Color8(40, 36, 32), "_e": Color8(78, 70, 63),
	"_h": Color8(158, 148, 136), "_H": Color8(178, 168, 155),
}
...
# 草须: 5 根又细又高的草叶, 高度不一, 叶间露天 (破掉草地硬顶边, 像泰拉瑞亚).
# g=嫩绿叶尖 G=深绿叶身 k=根部暗影. 长在草地上方那格 AIR 里, 根扎 row14 接草顶.
const _PLANT_GRASS := [
	"................",
	".......g........",
	".....g.G........",
...
const _PATTERN_MAP := {
	...
	CUTTING_BOARD: [_CUTTING_BOARD, _P_CUTTING_BOARD],
