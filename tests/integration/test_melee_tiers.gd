# 近战材质变种验收: 金/钻双刀/流星锤/战锤 注册齐全 + 高 tier = 高伤 + 图标自动换色。
# (长矛已删)
extends GutTest
...
func test_tier_icon_recolor_differs_from_base() -> void:
	...
	var base_tex = ItemsArt.get_icon("flail")
	var gold_tex = ItemsArt.get_icon("gold_flail")
	var dia_tex = ItemsArt.get_icon("diamond_flail")
	assert_not_null(gold_tex, "金流星锤图标生成成功")
	assert_not_null(dia_tex, "钻石流星锤图标生成成功")
	# 取中间一像素比较 (金属部分应换了色)
	...
	for y in range(0, ib.get_height()):
		...
		if diff:
			break
	assert_true(diff, "金流星锤图标该跟基础流星锤颜色不同")
