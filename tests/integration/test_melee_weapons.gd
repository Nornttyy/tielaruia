# 新近战类型验收: 长矛/双刀/流星锤/战锤 注册齐全 + 各自属性 (数据驱动手感)。
extends GutTest


func test_melee_weapons_registered() -> void:
	for id in ["spear", "dual_blade", "flail", "warhammer"]:
...
func test_melee_distinct_feel() -> void:
	var spear: Dictionary = ItemDB.get_def("spear")
	...
	# 长矛: 戳 + 超长射程
	assert_eq(String(spear.get("sword_style", "")), "thrust", "长矛是戳")
	assert_gt(float(spear.get("melee_reach_bonus", 0.0)), 20.0, "长矛戳得远")
	# 双刀: 攻速快 (cd 小)
