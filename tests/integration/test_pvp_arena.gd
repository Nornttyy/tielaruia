...
func test_arena_builds_floor_platforms_symmetric() -> void:
	...
	# 中央高台: 柱子石头
	assert_eq(cm.get_tile(cx, fy - 10), Tiles.STONE, "中央高台该有石柱")
	# 第一层平台 (dy=6, from=18): 左右镜像都该是木平台
	assert_eq(cm.get_tile(cx - 18, fy - 6), Tiles.WOOD_PLATFORM, "左侧平台在位")
	assert_eq(cm.get_tile(cx + 18, fy - 6), Tiles.WOOD_PLATFORM, "右侧平台在位")
	# 对称: 抽几个点, 中线两侧同 tile
	for d in [18, 30, 44]:
		assert_eq(cm.get_tile(cx - d, fy - 6), cm.get_tile(cx + d, fy - 6), "平台关于中心对称 (d=%d)" % d)
	# 出生点返回非零
