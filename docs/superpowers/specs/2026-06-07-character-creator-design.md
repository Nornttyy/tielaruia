...
@export var shirt_style: int = 0       # 0..15 (见 C 完整目录)
@export var pants_style: int = 0       # 0..16 (见 C 完整目录)
@export var cape_style: int = 0        # 0=无 1=短披风 2=长披风 3=蝴蝶翅膀 4=恐龙尾巴
...
0. **披风/背后层 (CAPE)** — 画在**最底 (身体后面)**: 按 `cape_style` 选 (0 无 / 1 短披风 / 2 长披风 / 3 蝴蝶翅膀 / 4 恐龙尾巴)。`0` 时整层透明。颜色 `cape_color` (翅膀=半透明亮色)。
...
- **衬衫款 `shirt_style` (16)**: 0 T恤 / 1 背带 / 2 连帽卫衣 / 3 海魂衫(蓝白条纹) / 4 法师袍(上) / 5 骑士胸甲 / 6 泳衣·背心式 / 7 泳衣·吊带式(女=比基尼上衣, 男=吊带) / 8 女仆上装(黑连衣裙上半 + 白围裙白领) / 9 水手服(上, 水手领) / 10 仙女装(上, 梦幻连衣裙上半) / 11 和服(上襟) / 12 忍者服(上, 黑劲装+面巾) / 13 太空服(上, 白宇航服+胸前仪表) / 14 机甲战甲(上, 金属板+能量线) / 15 恐龙连体衣(上, 带恐龙帽)。
- **裤子款 `pants_style` (17)**: 0 长裤 / 1 短裤 / 2 裙子 / 3 工装裤 / 4 公主裙(华丽蓬蓬裙) / 5 法师袍(下摆) / 6 骑士护腿 / 7 泳裤 / 8 泳裙(沙滩裤) / 9 女仆裙(黑裙 + 白蕾丝边) / 10 百褶裙(配水手服) / 11 仙女裙(下摆) / 12 和服(下摆) / 13 忍者裤 / 14 太空服(下) / 15 机甲护腿 / 16 恐龙连体衣(下)。
- **披风/背后层 `cape_style` (5)**: 0 无 / 1 短披风 / 2 长披风 / 3 蝴蝶翅膀(配仙女装, 半透明亮色) / 4 恐龙尾巴(配恐龙连体衣)。
- **发型 `hair_style` (4)**: 0 短发 / 1 长发 / 2 马尾 / 3 呆毛。
- **泳衣** = 泳衣上装 (shirt 6/7) + 泳衣下装 (pants 7/8) 自由搭, 共 2×2=4 种泳衣组合。
- **主题套装**: 法师袍 = shirt 4 + pants 5 (可再配长披风); 骑士盔甲 = shirt 5 + pants 6; 女仆装 = shirt 8 + pants 9; 水手服 = shirt 9 + pants 10; 仙女装 = shirt 10 + pants 11 + cape 3(蝴蝶翅膀); 和服 = shirt 11 + pants 12; 忍者服 = shirt 12 + pants 13; 太空服 = shirt 13 + pants 14; 机甲战甲 = shirt 14 + pants 15; 恐龙连体衣 = shirt 15 + pants 16 + cape 4(恐龙尾巴)。捏人 UI 各层独立选, 不强制成套。
...
**修改**:
- `scripts/art/player_art.gd` — 重构成分层 `build_sprite_frames(appearance)` (5 层: 披风/身体/裤子/衬衫/头发) + 男/女身体、4 发型、16 衬衫款、17 裤子款、5 披风/背后层款 (含蝴蝶翅膀/恐龙尾巴) ascii 模板
...
- **美术量很大** (24×48 画布 + 服装多 + 男女身材不同 + 高细节): 男+女身体各 7 帧 + 4 发型 + (16 衬衫款 + 17 裤子款) **× 男女 fit** + 5 披风/背后层款, 每个 24×48、跨帧对齐、还要明暗纹理。这是本功能的**主要工作量**。控制法: **分批上** (见实现顺序 —— 先框架+男版默认, 再女身体, 再日常款, 再主题套装+披风), 每批能独立跑测试上线; 走路帧腿/靴位置抽成共享坐标常量避免每帧手算; 男女身体共用头/臂/腿锚点行, 让头发+多数衣服只在宽窄上分 male/female-fit, 不全重画; 款式美术加进来不改框架代码 (纯加 ascii 模板 + 1 个 case)。**实现计划里每批款式数量要排到可交付的小块**, 不一口气画完。
...
3. C `player_art.gd` 分层重构 —— **分批**:
   ...
   - C4 泳衣 (2 上装 + 2 下装) + 背后层 (短/长披风 + 蝴蝶翅膀 + 恐龙尾巴)。
   - C5 主题套装 (法师袍 · 骑士甲 · 公主裙 · 女仆装 · 水手服 · 仙女装 · 和服 · 忍者服 · 太空服 · 机甲战甲 · 恐龙连体衣; 每套上+下, 美术量大, 实现计划再细分小批)。
   每批加完跑一次像素断言测试。
