...
- **美术量很大** (24×48 画布 + 服装多 + 男女身材不同 + 高细节): 男+女身体各 7 帧 + 4 发型 + (8 衬衫款 + 9 裤子款) **× 男女 fit** + 3 披风款, 每个 24×48、跨帧对齐、还要明暗纹理。这是本功能的**主要工作量**。控制法: **分批上** (见实现顺序 —— 先框架+男版默认, 再女身体, 再日常款, 再主题套装+披风), 每批能独立跑测试上线; 走路帧腿/靴位置抽成共享坐标常量避免每帧手算; 男女身体共用头/臂/腿锚点行, 让头发+多数衣服只在宽窄上分 male/female-fit, 不全重画; 款式美术加进来不改框架代码 (纯加 ascii 模板 + 1 个 case)。**实现计划里每批款式数量要排到可交付的小块**, 不一口气画完。
