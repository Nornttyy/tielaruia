...
@export var shirt_style: int = 0       # 0..8 (含 2 款泳衣上装 + 女仆上装, 见 C)
@export var pants_style: int = 0       # 0..9 (含 2 款泳衣下装 + 女仆裙, 见 C)
...
- **衬衫款 `shirt_style` (9)**: 0 T恤 / 1 背带 / 2 连帽卫衣 / 3 海魂衫(蓝白条纹) / 4 法师袍(上) / 5 骑士胸甲 / 6 泳衣·背心式 / 7 泳衣·吊带式(女=比基尼上衣, 男=吊带) / 8 女仆上装(黑连衣裙上半 + 白围裙白领)。
- **裤子款 `pants_style` (10)**: 0 长裤 / 1 短裤 / 2 裙子 / 3 工装裤 / 4 公主裙(华丽蓬蓬裙) / 5 法师袍(下摆) / 6 骑士护腿 / 7 泳裤 / 8 泳裙(沙滩裤) / 9 女仆裙(黑裙 + 白蕾丝边)。
- **披风款 `cape_style` (3)**: 0 无 / 1 短披风 / 2 长披风。
- **发型 `hair_style` (4)**: 0 短发 / 1 长发 / 2 马尾 / 3 呆毛。
- **泳衣** = 泳衣上装 (shirt 6/7) + 泳衣下装 (pants 7/8) 自由搭, 共 2×2=4 种泳衣组合。
- **主题套装**: 法师袍 = shirt 4 + pants 5 (可再配长披风); 骑士盔甲 = shirt 5 + pants 6; 女仆装 = shirt 8 + pants 9。捏人 UI 各层独立选, 不强制成套。
...
**修改**:
- `scripts/art/player_art.gd` — 重构成分层 `build_sprite_frames(appearance)` (5 层: 披风/身体/裤子/衬衫/头发) + 男/女身体、4 发型、9 衬衫款、10 裤子款、3 披风款 ascii 模板
...
- **美术量很大** (24×48 画布 + 服装多 + 男女身材不同 + 高细节): 男+女身体各 7 帧 + 4 发型 + (9 衬衫款 + 10 裤子款) **× 男女 fit** + 3 披风款, 每个 24×48、跨帧对齐、还要明暗纹理。这是本功能的**主要工作量**。控制法: **分批上** (见实现顺序 —— 先框架+男版默认, 再女身体, 再日常款, 再主题套装+披风), 每批能独立跑测试上线; 走路帧腿/靴位置抽成共享坐标常量避免每帧手算; 男女身体共用头/臂/腿锚点行, 让头发+多数衣服只在宽窄上分 male/female-fit, 不全重画; 款式美术加进来不改框架代码 (纯加 ascii 模板 + 1 个 case)。**实现计划里每批款式数量要排到可交付的小块**, 不一口气画完。
...
3. C `player_art.gd` 分层重构 —— **分批**:
   ...
   - C5 主题套装 (法师袍 上+下 · 骑士胸甲+护腿 · 公主裙 · 女仆装 上+下)。
   每批加完跑一次像素断言测试。
