...
@export var shirt_style: int = 0       # 0..18 (见 C 完整目录)
@export var pants_style: int = 0       # 0..19 (见 C 完整目录)
@export var cape_style: int = 0        # 0=无 1=短披风 2=长披风 3=蝴蝶翅膀 4=恐龙尾巴 5=小狗尾巴 6=兔子尾巴(毛球)
@export var chest_size: int = 1        # 仅 gender=1(女) 生效: 0..5 (0 最小/平, 5 最大); 男忽略
...
**画布加大到 24×48 (用户要「很多细节」)**: 从 12×24 翻倍到 **24×48 像素**, 给衣服褶皱/盔甲纹理/脸部细节留地方。
- **游戏里不变大**: 玩家场景挂 `AnimatedSprite2D` 的 `scale` 减半 (原来啥缩放 → ×0.5), 让小人在世界里占的高度/宽度、脚底落点**和现在一样**; 碰撞体 (CollisionShape2D) **完全不动**。只是贴图像素更密。
- `PixelArt.build_sprite_frames` 按 ascii 网格行列数定贴图尺寸, 改大网格即可; 但要核对 `player_art.gd` 调用处 + 玩家场景里跟 sprite 尺寸/居中相关的设置 (脚底锚点、名字牌/血条偏移), 保持世界坐标不变。

**美术细节标准 (用户要「很多细节」, 见项目反馈「暖色+可识别形状」)**:
- 每件衣服/身体**都要明暗** (高光 + 阴影, 不是一块纯色平涂)。
- **可识别的纹理/形状**: 毛衣的针织纹、盔甲的板甲分块+铆钉、海魂衫的条纹、衬衫的纽扣/领口/腰带/缝线、法师袍的星纹、披风的褶皱、头发一缕缕带高光、脸有鼻尖/睫毛。
- 色板**偏暖** (暖灰棕/黄绿/暖橙/温红棕), 不要冷蓝绿; 不要随机散点当细节。
...
0. **披风/背后层 (CAPE)** — 画在**最底 (身体后面)**: 按 `cape_style` 选 (0 无 / 1 短披风 / 2 长披风 / 3 蝴蝶翅膀 / 4 恐龙尾巴 / 5 小狗尾巴 / 6 兔子尾巴)。`0` 时整层透明。颜色 `cape_color` (翅膀=半透明亮色)。
1. **身体层 (BODY)**: 按 `gender` 选模板组。只画**皮肤 + 靴**, 躯干/腿留空 (`.`)。男/女**各一套完整全帧** (idle_a/idle_b/walk_a/walk_c/jump/fall/hurt; walk_b=idle_a 复用)。
   ...
   - **眼睛 = 眼白 + 眼珠**: 眼睛**不再是一团黑**。眼白固定白色 (`PALETTE` 加 `W白`), 眼珠(虹膜)用 `eye_color` (可改)。24×48 画布下侧面单眼约 3-4px (眼白 + 眼珠 + 深色描边/睫毛), 画得清楚。`m`(嘴) 放侧脸位置。
   - **男女身材比例不同 (用户要大区别)**: 男版 = 侧面、肩宽、躯干直; 女版 = **重画身材比例** —— 肩略窄、腰更收、腿型略不同, 脸加睫毛/大眼一眼区分。
   - **女版胸部大小可调** (`chest_size`, 仅 gender=1 生效): **6 档 0..5**, 侧面胸口前凸随档位每档约 +1px (0≈平 → 5≈最大约 5px), 全程**卡通风、不夸张**, 即使最大档也是可爱风。女身体躯干那几行按 `chest_size` 取对应 ascii 变体 (6 份)。男版忽略此项。
   - **关键约束 (省美术 + 保对齐)**: 两套身体**同画布 (24×48)**, 且**头部行段、手臂行、腿/靴锚点行保持一致** —— 这样头发层和大多数衣服层在男女上都能用同一锚点对齐, 不必每件衣服画两遍。只有**躯干/腿宽窄**男女不同。
...
- **衬衫款 `shirt_style` (19)**: 0 T恤 / 1 背带 / 2 连帽卫衣 / 3 海魂衫(蓝白条纹) / 4 法师袍(上) / 5 骑士胸甲 / 6 泳衣·背心式 / 7 泳衣·吊带式(女=比基尼上衣, 男=吊带) / 8 女仆上装(黑连衣裙上半 + 白围裙白领) / 9 水手服(上, 水手领) / 10 仙女装(上, 梦幻连衣裙上半) / 11 和服(上襟) / 12 忍者服(上, 黑劲装+面巾) / 13 太空服(上, 白宇航服+胸前仪表) / 14 机甲战甲(上, 金属板+能量线) / 15 恐龙连体衣(上, 带恐龙帽) / 16 小狗连体衣(上, 带狗耳帽) / 17 兔子连体衣(上, 带兔耳帽; 可爱萌系, 非夜店兔女郎) / 18 木乃伊(上, 缠绕绷带)。
- **裤子款 `pants_style` (20)**: 0 长裤 / 1 短裤 / 2 裙子 / 3 工装裤 / 4 公主裙(华丽蓬蓬裙) / 5 法师袍(下摆) / 6 骑士护腿 / 7 泳裤 / 8 泳裙(沙滩裤) / 9 女仆裙(黑裙 + 白蕾丝边) / 10 百褶裙(配水手服) / 11 仙女裙(下摆) / 12 和服(下摆) / 13 忍者裤 / 14 太空服(下) / 15 机甲护腿 / 16 恐龙连体衣(下) / 17 小狗连体衣(下) / 18 兔子连体衣(下) / 19 木乃伊(下, 绷带)。
- **披风/背后层 `cape_style` (7)**: 0 无 / 1 短披风 / 2 长披风 / 3 蝴蝶翅膀(配仙女装, 半透明亮色) / 4 恐龙尾巴(配恐龙连体衣) / 5 小狗尾巴 / 6 兔子尾巴(圆毛球)。
- **发型 `hair_style` (4)**: 0 短发 / 1 长发 / 2 马尾 / 3 呆毛。
- **泳衣** = 泳衣上装 (shirt 6/7) + 泳衣下装 (pants 7/8) 自由搭, 共 2×2=4 种泳衣组合。
- **主题套装**: 法师袍 = shirt 4 + pants 5 (可再配长披风); 骑士盔甲 = shirt 5 + pants 6; 女仆装 = shirt 8 + pants 9; 水手服 = shirt 9 + pants 10; 仙女装 = shirt 10 + pants 11 + cape 3(蝴蝶翅膀); 和服 = shirt 11 + pants 12; 忍者服 = shirt 12 + pants 13; 太空服 = shirt 13 + pants 14; 机甲战甲 = shirt 14 + pants 15; 恐龙连体衣 = shirt 15 + pants 16 + cape 4(恐龙尾巴); 小狗装 = shirt 16 + pants 17 + cape 5(小狗尾巴); 兔子装 = shirt 17 + pants 18 + cape 6(兔子尾巴); 木乃伊装 = shirt 18 + pants 19。捏人 UI 各层独立选, 不强制成套。
...
2. **CharacterCreatorPanel** (捏人): 左侧**活预览** (一个 `AnimatedSprite2D` 放大, 播 idle/walk 切换) + 右侧选项:
   ...
   - **胸部大小** (滑条 0..5, 或 `◀ 数字 ▶`): **仅性别=女时显示**, 切到男时整行隐藏。
...
- `test_player_art_appearance.gd`: `build_sprite_frames(appearance)` 给定颜色 → 取某已知坐标像素 (如衬衫格) 断言等于所选 shirt_color; 换 hair_style 头顶像素变化; 默认 appearance 画出的 idle 第 0 帧与**新侧面基准**逐格一致 (锁住默认款不被后续改歪); 侧脸只有一只眼 (断言朝右那侧有眼、另一侧无); 眼睛区域**同时有眼白(白)和眼珠(=eye_color)**; 改 `eye_color` 眼珠像素跟着变; 女版 `chest_size` 0..5 六档胸口前凸像素数递增 (0 最平、5 最大), 男版不受 `chest_size` 影响。
...
**修改**:
- `scripts/art/player_art.gd` — 重构成分层 `build_sprite_frames(appearance)` (5 层: 披风/身体/裤子/衬衫/头发) + 男/女身体、4 发型、19 衬衫款、20 裤子款、7 披风/背后层款 (含翅膀/恐龙·小狗·兔子尾巴) ascii 模板
...
- 玩家创建处 (player 场景实例化时) — `AnimatedSprite2D.sprite_frames = PlayerArt.build_sprite_frames(current.appearance_dict())` + **`AnimatedSprite2D.scale` 减半** (因画布 12×24→24×48, 保世界尺寸/脚底锚点不变); 碰撞体不动
- 玩家场景 (`.tscn`) / 任何按旧 12×24 sprite 尺寸算偏移的地方 (名字牌/血条/粒子中心) — 核对世界坐标不变
- i18n 语言表 (`scripts/autoload/locale.gd` 或 4 语言资源) — 加捏人/选角色 UI 文字 key

## 风险

- **美术量很大** (24×48 画布 + 服装多 + 男女身材不同 + 高细节): 男+女身体各 7 帧 + 4 发型 + (19 衬衫款 + 20 裤子款) **× 男女 fit** + 7 披风/背后层款, 每个 24×48、跨帧对齐、还要明暗纹理。这是本功能的**主要工作量**。控制法: **分批上** (见实现顺序 —— 先框架+男版默认, 再女身体, 再日常款, 再主题套装+披风), 每批能独立跑测试上线; 走路帧腿/靴位置抽成共享坐标常量避免每帧手算; 男女身体共用头/臂/腿锚点行, 让头发+多数衣服只在宽窄上分 male/female-fit, 不全重画; 款式美术加进来不改框架代码 (纯加 ascii 模板 + 1 个 case)。**实现计划里每批款式数量要排到可交付的小块**, 不一口气画完。
...
3. C `player_art.gd` 分层重构 —— **分批**:
   - C1 5 层框架 + **24×48 画布** + 玩家场景 scale 减半 (世界尺寸/碰撞不变) + **侧面单眼男身体** (眼白+眼珠 `eye_color`, 带明暗) + 默认款 (短发/T恤/长裤/无披风) + 逐格基准测试 (锁住新侧面基准)。
   - C2 女身体 (重画身材比例 + `chest_size` 0..5 六档躯干变体) + 4 发型 + 颜色替换跑通; 默认款做出女版 fit。
   - C3 日常款 (背带/连帽/海魂衫 · 短裤/裙子/工装裤), 每款男女 fit 各一版。
   - C4 泳衣 (2 上装 + 2 下装) + 背后层 (短/长披风 + 蝴蝶翅膀 + 恐龙/小狗/兔子尾巴)。
   - C5 主题套装 (法师袍 · 骑士甲 · 公主裙 · 女仆装 · 水手服 · 仙女装 · 和服 · 忍者服 · 太空服 · 机甲战甲 · 恐龙/小狗/兔子连体衣 · 木乃伊; 每套上+下, 美术量大, 实现计划再细分小批)。
   每批加完跑一次像素断言测试。
